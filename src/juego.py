...
class FlorkWindow(QWidget):
    def __init__(self):
        ...
        icon = QIcon('assets/images/icon.png')  # Reemplaza 'ruta/a/tu/icono.png' con la ruta de tu icono
    ...
    def create_ready_page(self,n,aut,estadoActual):
        ...
        if estadoActual not in aut.estado_aceptacion:
            ...
            left_button_rect.clicked.connect(lambda: self.restartName(n))
    ...
    def restartName(self, n):
        ...
        msg.setIconPixmap(QPixmap('assets/images/question.png').scaled(80,80))  # Elimina el icono predeterminado
        msg.setWindowTitle("Reiniciar juego")
        msg.setText("¿Desea cambiar el nombre de Flork?")
        ...
        yes_button.setStyleSheet("background-color: #70BAFF; color: white;")
        yes_button.setFont(QFont("Gill Sans MT Condensed", 12))
        no_button = msg.addButton('No', QMessageBox.NoRole)
        no_button.setStyleSheet("background-color: #FF3B41; color: white;")
        no_button.setFont(QFont("Gill Sans MT Condensed", 12))
        yes_button.clicked.connect(self.restartGame)
        no_button.clicked.connect(lambda: self.startGame(n))
        msg.exec_()

    def restartGame(self):
    # Reinicia el juego volviendo a la página de introducción
        self.page_intro = self.create_intro_page()
        self.stacked_widget.addWidget(self.page_intro)
    ...
    def validar_nombre(self):
        ...
        if exp.validateName(nombre):
            ...
            msg = QMessageBox(self)
            msg.setIconPixmap(QPixmap('assets/images/error.png').scaled(80,80))  # Elimina el icono predeterminado
            msg.setWindowTitle("Error")
            msg.setText("La cadena debe cumplir con las siguientes condiciones: \n\n*Solo se aceptan letras  (a-z o A-Z). \n*La primer letra debe ser mayúscula (A-Z). \n*La longitud de la cadena debe ser mayor a 2 caracteres. \n\nEjemplo: (Diana, Carlos, Danna)")
            msg.setFont(QFont("Gill Sans MT Condensed", 12))
            msg.setStyleSheet("background-color: white; color: black;")
            
            ok_button = msg.addButton(QMessageBox.Ok)
            ok_button.setStyleSheet("background-color: #FF3B41; color: white;")
            ok_button.setFont(QFont("Gill Sans MT Condensed", 12))
            # Conectar la señal accepted al botón "OK"
...
class NuevoFinalDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setFont(QFont("Gill Sans MT Condensed", 12))
        self.setStyleSheet("background-color: #70BAFF; color: black;")
        ...
        self.accept_button.setStyleSheet("background-color: #FF3B41; color: white;")
        #self.accept_button.setFont(QFont("Gill Sans MT Condensed", 12))
    ...
    def validar_final(self):
            ...
            if gr.validar_cadena(new_final):
                ...
                msg.setIconPixmap(QPixmap('assets/images/error.png').scaled(80,80))  # Elimina el icono predeterminado
                msg.setWindowTitle("Error")
                msg.setText("El final sugerido no cumple con la gramática establecida ya que debe empezar con la cadena 'Yo creo que el final puede mejorar con '. ¿Desea seguir editando?")
                msg.setFont(QFont("Gill Sans MT Condensed", 12))
                msg.setStyleSheet("background-color: white; color: black;")
